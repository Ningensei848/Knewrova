...
get_lock_age() {
    ...
    # stat (Linux/Git Bash) または date -r (Mac/BSD) でmtimeを取得
    local mtime
...
mk_lock() {
    ...
    # アトミックなディレクトリ作成によるロック取得
    if mkdir "$lock_path" 2>/dev/null; then
        local user="${USER_NAME:-$USER}"
        local now; now="$(date +%s)"
        echo "{\"user\":\"$user\",\"host\":\"$HOSTNAME\",\"pid\":\"$PID\",\"epoch\":$now}" > "$lock_path/meta.json"
        return 0
    fi
    return 1
}
...
# --- Policy Check (Fast-Forward & Tag protection) ---
